...
// Cache transporter across invocations
let transporter;
...
function buildLeadEmailText({ name, email, phone, purpose, pageUrl }) {
  return (
    `Hi Team,\n\n` +
    `A visitor has submitted their details on the CloudSeals website. Please find the information below:\n\n` +
    `Customer Details\n` +
    `----------------\n` +
    `Name    : ${name}\n` +
    `Email   : ${email}\n` +
    ...
    `Page    : ${pageUrl || "-"}\n\n` +
    `Kindly reach out to the customer at your earliest convenience.\n\n` +
    `Regards,\n` +
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!name || !email || !phone || !purpose) {
      return res.status(400).json({ ok: false, error: "Missing name/email/phone/purpose" });
    }
    if (!isGmail(email)) {
    ...
    const subject = `New Website Enquiry - ${prettyPurpose(purpose)} | ${name}`;
    const text = buildLeadEmailText({ name, email, phone, purpose, pageUrl });
